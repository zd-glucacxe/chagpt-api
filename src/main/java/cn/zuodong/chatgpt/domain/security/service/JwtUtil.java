...
/**
 * @author zuodong
 * @create 2023-12-05 18:17
 */

public class JwtUtil {

    // 创建默认的秘钥和算法，供无参的构造方法使用
    // 默认的Base64编码的密钥和签名算法（HS256）
    ...
    /**
     * 这里就是产生jwt字符串的地方  encode 方法用于生成JWT
     * jwt字符串包括三个部分
     *  1. header
     *      -当前字符串的类型，一般都是“JWT”
    ...
    // decode 方法用于解析JWT，返回 Claims 对象。 使用JWT的签名密钥进行解析。
    ...
    public boolean isVerify(String jwtToken) {
        ...
        switch (signatureAlgorithm) {
            case HS256:
                algorithm = Algorithm.HMAC256(Base64.decodeBase64(base64EncodedSecretKey));  //根据签名算法创建相应的 Algorithm 对象
                break;
            default:
                throw new RuntimeException("不支持该算法");
        }
        JWTVerifier verifier = JWT.require(algorithm).build();  // 使用 JWTVerifier 进行验证，如果验证通过，则返回 true
